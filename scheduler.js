...
// Tarea 3: La Granjita (Minutos 0, 2 y 5, entre las 8am y las 7pm) 
cron.schedule('0,2,5 8-19 * * *', () => { 
    console.log(`[${new Date().toLocaleTimeString()}] 🚜 Despachando Scraper La Granjita...`); 
    exec('node js/scrapers/la-granjita-scraper.js', (error, stdout, stderr) => { 
        if (error) console.error(`🚨 Error La Granjita: ${error.message}`); 
        if (stdout) console.log(stdout.trim()); 
    }); 
}); 
